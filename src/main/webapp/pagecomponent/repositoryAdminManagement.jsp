...
<script>
                    ...
                    columns: [
                        ...
                        },
                        {
                            ...
                            formatter: function (value, row, index) {
                                ...
                                var r = '<button class="btn btn-warning btn-sm resetpw"><span>重置密码</span></button>';
                                var fun = '';
                                return s + ' ' + d + ' ' + r;
                            },
                            events: {
    ...
    // 重置仓库管理员密码
    function resetRepositoryAdminPassword() {
        $('#resetpw_confirm').click(function () {
            debugger
            var data = {
                "repositoryAdminID": selectID,
                "encodePwd": $.md5(selectID.toString())
            }

            // ajax
            $.ajax({
                type: "GET",
                url: "repositoryAdminManage/resetRepositoryAdminPassword",
                dataType: "json",
                contentType: "application/json",
                data: data,
                success: function (response) {
                    $('#resetpwWarning_modal').modal("hide");
                    var type;
                    var msg;
                    var append = '';
                    if (response.result == "success") {
    ...
    function addRepositoryAdminAction() {
        ...
        $('#add_modal_submit').click(function () {
            $('#repositoryAdmin_form').data('bootstrapValidator').validate();
...
<!-- 重置密码提示模态框 -->
<div class="modal fade" id="resetpwWarning_modal" table-index="-1"
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row">
                    ...
                    <div class="col-md-8 col-sm-8">
                        <h3>是否确认重置该条仓库管理员密码</h3>
                        <p>(注意：重置后密码为该用户的<font color="red">用户编码</font>)</p>
                    </div>
                </div>
            </div>
            <div class="modal-footer">
                ...
                <button class="btn btn-danger" type="button" id="resetpw_confirm">
                    <span>确认重置</span>
                </button>
            </div>
        </div>
